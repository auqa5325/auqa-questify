...
subject_code = st.text_input("Subject Code", "OS")
subject_name = st.text_input("Subject Name", "Operating Systems")
department = st.text_input("Department", "Computer Science and Engineering")
semester = st.text_input("Semester", "V")
...
_easy = col1.number_input("Easy ratio", 0.0, 1.0, 0.25, 0.01)
_medium = col2.number_input("Medium ratio", 0.0, 1.0, 0.50, 0.01)
_hard = col3.number_input("Hard ratio", 0.0, 1.0, 0.25, 0.01)
...
if st.button("Retrieve contexts for selected units") and st.session_state.units_parsed:
    if not selected_units:
        ...
        for unit in selected_units:
            ...
            query = (" ".join(unit.get("topics", [])) or unit_name)
            ...
            try:
                if subject_code:
                    bm25_query = {
                        ...
                        "query": {
                            "bool": {
                                "filter": [{"term": {"course_id": subject_code}}],
...
llm_template_a = """
You are an exam question author writing concise short-answer questions for PART A.
Generate {N_A} short questions (2 marks each) for PART A from Unit {UNIT_NO} — "{UNIT_NAME}".
Guidance for tone and style:
- Use typical academic exam phrasing (vary lead verbs: Define, State, Give, Why, What, List, Identify, Briefly explain).
- Keep each question short and focused so the expected answer is brief (1–3 sentences).
- Difficulty should come from reasoning required, not length.
- Where appropriate map the question to a CO from the course.
- Vary wording — do NOT always start with \"What is\".
Examples of acceptable starters: \"Define...\", \"State two reasons why...\", \"Give the difference between...\", \"Why does... occur?\", \"Identify the ... and justify briefly.\"

### Return ONLY a JSON array (no commentary), each object with fields:
[{{"question_text":"...", "difficulty":"Easy|Medium|Hard", "blooms_level":"...", "CO":"COx", "marks":2,
   "part":"A", "q_no":<int>, "subpart":null, "context_ref":"..."}}]
"""

llm_template_b = """
You are an exam question author writing medium-length questions for PART B.
Generate {N_B} subpart items (13 marks each) for PART B from Unit {UNIT_NO} — "{UNIT_NAME}".
Guidance for tone and style:
- Use academic phrasing suitable for 8–13 mark answers: Discuss, Explain, Compare, Derive, Trace, Illustrate, Design, Analyze, Show with steps.
- Questions may be single-part or have (a)/(b) structure; allow the model to use subdivisions when appropriate (but indicate if the model creates subparts).
- Demand moderate solution effort: derivation, short design, analysis, or structured explanation with steps and small calculations when relevant.
- Use varied verbs and realistic exam framing (e.g., \"Discuss in detail...\", \"Compare X and Y with examples\", \"Trace the sequence of events when...\").
- Map questions to COs where appropriate.

### Return ONLY a JSON array (no commentary), each object with fields:
[{{"question_text":"...", "difficulty":"Easy|Medium|Hard", "blooms_level":"...", "CO":"COx", "marks":13,
   "part":"B", "q_no":<int>, "subpart":null}}]
"""

llm_template_c = """
You are an exam question author writing deep, scenario/problem-based questions for PART C.
Generate ONE HARD question for PART C subpart {SUB_ID} (marks={MARKS}) from Unit {UNIT_NO} — "{UNIT_NAME}".
Guidance for tone and style:
- Write a realistic scenario or problem statement that requires multi-step reasoning, calculations or design decisions (like typical university end-semester questions).
- Frame it as: set-up / specific requirements / tasks to perform / deliverables expected (e.g., design an addressing scheme + show calculations; analyze performance with numbers; derive fragmentation parameters and justify choices).
- Include any numerical parameters needed for calculation. Specify what the student must show (e.g., subnet boundaries, fragment sizes, formulas, diagrams, brief justification).
- Avoid generic \"explain\" style; give a concrete task (e.g., \"Consider..., design..., compute..., show...\").
- Strictly HARD difficulty and intended for students well-versed in the topic.

### Return ONLY a single JSON object (no commentary) in this exact form:
{{"question_text":"...", "difficulty":"Hard", "blooms_level":"...", "CO":"COx", "marks":{MARKS},
    ...
    if st.button("🚀 Generate Full Question Paper"):
        ...
        if n_units == 0:
            ...
            for idx, unit_no in enumerate(selected_unit_nos):
                ...
                if n_a > 0:
                    llm_guidance_a = llm_template_a.replace("{N_A}", str(n_a)) \
                    .replace("{UNIT_NO}", str(unit_no)) \
                    .replace("{UNIT_NAME}", unit_name) \
                    .replace("{E}", f"{_e:.2f}") \
                    .replace("{M}", f"{_m:.2f}") \
                    .replace("{H}", f"{_h:.2f}")
                    prompt_obj = {
                        "llm_guidance": llm_guidance_a,
                        "paper_context": {
                            "unit_no": unit_no,
                            "unit_name": unit_name,
            ...
            for idx, unit_no in enumerate(selected_unit_nos):
                ...
                if n_b_subparts > 0:
                    llm_guidance_b = llm_template_b.replace("{N_B}", str(n_b_subparts)) \
                    .replace("{UNIT_NO}", str(unit_no)) \
                    .replace("{UNIT_NAME}", unit_name) \
                    .replace("{E}", f"{_e:.2f}") \
                    .replace("{M}", f"{_m:.2f}") \
            ...
            for sub_id, marks in zip(["a", "b"], [8, 7]):
                ...
                context_text = "\n\n".join([t for _, t, _ in chunks])
                context_refs = [p for _, _, p in chunks]
                llm_guidance_c = llm_template_c.replace("{SUB_ID}", str(sub_id)) \
                .replace("{MARKS}", str(marks)) \
                .replace("{UNIT_NO}", str(chosen_unit)) \
                .replace("{UNIT_NAME}", unit_name)
                prompt_obj = {
                    "llm_guidance": llm_guidance_c,
                    "paper_context": {
                        "unit_no": chosen_unit,
                        "unit_name": unit_name,
                        "context": context_text,
                ...
                if model_config:
                    ...
                    gtext = clean_json_output(gtext)
                    try:
                        parsed = json.loads(gtext)

                        # Support both a JSON object and a 1-element JSON array
                        if isinstance(parsed, list):
                            if len(parsed) == 0:
                                raise ValueError("Model returned an empty list for Part-C.")
                            if len(parsed) > 1:
                                # Model returned more than one object — keep first and warn
                                st.warning(f"Model returned {len(parsed)} objects for Part-C {sub_id}; using the first one.")
                            obj = parsed[0]
                        elif isinstance(parsed, dict):
                            obj = parsed
                        else:
                            raise ValueError("Unexpected JSON type returned by model for Part-C (expected object or single-element list).")

                        # Ensure obj is a dict before using setdefault
                        if not isinstance(obj, dict):
                            raise ValueError("Parsed Part-C item is not a JSON object.")
                        ...
                        st.text_area("LLM output (raw)", gtext, height=300)
...
def export_final_paper(final_paper_list: list,
    ...
    p = doc.add_paragraph(f"{subject_name} - {department} - Semester: {semester}  Reg: {regulation}")
    ...
    return {"csv": str(csv_path), "docx": str(docx_path)}
